...
const ProjectItem: React.FC<ProjectItemProps> = ({
  ...
  return <Card className="h-full flex flex-col card-hover">
      ...
      <CardContent className="flex-grow">
        ...
        {hasAchievements && <Collapsible open={isOpen} onOpenChange={setIsOpen} className="mt-2">
            ...
            <CollapsibleContent className="mt-2">
              <ul className="space-y-2 pl-4">
                {achievements.map((achievement, i) => <li key={i} className="text-sm relative pl-2">
      ...
      <CardFooter className="flex gap-2">
        {githubLink && <Button variant="outline" size="sm" asChild className="gap-1">
            <a href={githubLink} target="_blank" rel="noopener noreferrer">
              <Github size={14} />
              <span className="font-medium">Code</span>
            </a>
          </Button>}
        {demoLink && <Button variant="default" size="sm" asChild className="gap-1">
